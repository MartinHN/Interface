...
void metaBalls::update(int w, int h){
    ...
    switch(mode){
        ...
        case 0:{
            for (int i = 0 ; i<dpoints.size() ; i++){
                if(i<points.size()){
                    ...
                    if(points[i].lifetime<lifeTime*ofGetFrameRate()){
                        points[i].lifetime++;
                    }

                }
                else{
                    points.push_back(Meta(dpoints[i],rayon,lifeTime*ofGetFrameRate()));
                    //            if(points.size() == 0){
                    //
        ...
        case 1:{
            //Emit
            if((emitrate==0 || ofGetElapsedTimeMillis()-lastT> 1000./emitrate) && dpoints.size()>0 && points.size()<1000){
                ofVec3f v(vInit,0);
                ...
                for (int i = 0 ; i< dpoints.size() ; i++){
                    ...
                    if(emitrate==0 && points.size()>1){
                        ...
                        if(dist>0 ){
                            if(dist< .2){
                                float pointStep = 1.0/200;

                                auto num = dist;
                                ofVec3f v = (dpoints[i]-last)/dist;
                                ofVec3f n (v.y,-v.x);
                                for(float i = 0;i<dist ; i+=pointStep){
                                    auto p = last+ i*v +noise*(ofRandom(2.0)-1)*n;
                                    points.push_back(Meta(p,(ofRandom(noiseR)+1)*rayon,lifeTime*ofGetFrameRate(),tmpv,i));
                                }

                            }
                            else{
                                points.push_back(Meta(dpoints[i],(ofRandom(noiseR)+1)*rayon,lifeTime*ofGetFrameRate(),tmpv,i));
                            }
                        }
                    }
                    else{
