...
ofPolyline ofPolyline::getSmoothed(int smoothingSize, float smoothingShape) const {
    ...
    float margin =0.01; // hack to avoid smoothing sharp edges
    for(int i = 0; i < n; i++) {
        if(result[i].x <margin) result[i].x=0;
        if(result[i].y <margin) result[i].y=0;
        if(result[i].x >1-margin) result[i].x=1;
        if(result[i].y >1-margin) result[i].y=1;
        //        if(result[i].x>margin && result[i].y>margin
        //           &&result[i].x<1-margin && result[i].y<1-margin){
        else{
            ...
            for(int j = 1; j < smoothingSize; j++) {
                ...
                if(leftPosition >= 0 && rightPosition<n) {
                    auto p = points[leftPosition];

                    if((p.x <margin )|| (p.y <margin) ||
                       (p.x >1-margin) || (p.y >1-margin)) {
                        isBorder = true;
                    }
                    p = points[rightPosition];
                    if((p.x <margin )|| (p.y <margin) ||
                       (p.x >1-margin) || (p.y >1-margin)) {
                        isBorder = true;
                    }
